...
export class BrandComponent implements OnInit {
  ...
  validateField(field: string) {
    switch (field) {
      case "code":
        this.codeError =
          this.newBrand.code === "" || this.newBrand.code.length > 50;
        break;
      case "name":
        this.nameError =
          this.newBrand.name === "" || this.newBrand.name.length > 50;
        break;
      case "description":
        this.descriptionError =
          this.newBrand.description === "" ||
          this.newBrand.description.length > 50;
        break;
    }
  }
  ...
  onRowEditCancel(brand: BrandData) {
    ...
    if (brandToUpdate) {
      ...
      brandToUpdate.description =
        this.clonedBrands[brand.id as number].description;
      brandToUpdate.lastModifiedBy =
        this.clonedBrands[brand.id as number].lastModifiedBy;
      brandToUpdate.lastModifiedDate =
        this.clonedBrands[brand.id as number].lastModifiedDate;
  ...
  showDialog() {
    ...
    //   const newCode =
    //     "BR" + (this.brand.length + 1).toString().padStart(3, "0");
